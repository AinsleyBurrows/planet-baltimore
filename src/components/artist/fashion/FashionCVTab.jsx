...
import {
  GraduationCap, Briefcase, Sparkles, Handshake, Trophy, Newspaper, ShoppingBag,
...
const SECTIONS = [
  { key: 'education', label: 'Education', icon: GraduationCap, fields: ['year', 'degree', 'institution'], primary: 'degree', secondary: ['institution'] },
  { key: 'design_experience', label: 'Design Experience', icon: Briefcase, fields: ['year', 'role', 'company', 'location'], primary: 'role', secondary: ['company', 'location'] },
  { key: 'collections', label: 'Collections & Runway', icon: Sparkles, fields: ['year', 'title', 'venue', 'location'], primary: 'title', secondary: ['venue', 'location'] },
  ...
  { key: 'press', label: 'Press & Editorials', icon: Newspaper, fields: ['year', 'title', 'publication', 'url'], primary: 'title', secondary: ['publication'], link: 'url' },
  { key: 'stockists', label: 'Stockists', icon: ShoppingBag, fields: ['name', 'location', 'url'], primary: 'name', secondary: ['location'], link: 'url', hideYear: true },
];

const FIELD_PLACEHOLDERS = {
  year: 'Season / Year',
  degree: 'BFA Fashion Design',
  institution: 'Parsons, Central Saint Martins…',
  role: 'Senior Designer, Creative Director…',
  company: 'Maison Margiela, Studio Nicholson…',
  location: 'City, Country',
  title: 'Collection / Project name',
  venue: 'NYFW, Paris FW, Presentation…',
  partner: 'Collaboration partner / brand',
  organization: 'CFDA, Vogue, ANDAM…',
  publication: 'Vogue, Harper\'s Bazaar, i-D…',
  name: 'SSENSE, Net-a-Porter, Dover Street…',
  url: 'https://…'
};
            ...
            items.map((item, i) => (
              <div key={i} className="flex items-start gap-3 group">
                <div className="flex-1 flex flex-wrap gap-x-3 gap-y-0.5">
                  {!section.hideYear && item.year && <span className="text-xs font-bold text-muted-foreground w-16 flex-shrink-0 mt-0.5">{item.year}</span>}
...
const EMPTY = {
  artist_statement: '',
  education: [], design_experience: [], collections: [], collaborations: [],
  awards: [], press: [], stockists: []
};

export default function FashionCVTab({ artistId, isOwner, ownerId }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-bold text-foreground">Designer CV / Bio</h2>
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Designer Statement</p>
        {editing ? (
          <textarea className="w-full px-3 py-2 rounded-lg border border-input bg-transparent text-sm resize-none focus:outline-none focus:ring-1 focus:ring-ring min-h-[120px]"
            placeholder="Write your designer statement / brand ethos…"
