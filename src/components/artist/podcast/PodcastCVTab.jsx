...
import {
  GraduationCap, Briefcase, Mic, Users, Headphones, Calendar, Trophy, Newspaper, BadgeDollarSign,
...
const SECTIONS = [
  { key: 'education', label: 'Education & Background', icon: GraduationCap, fields: ['year', 'degree', 'institution'], primary: 'degree', secondary: ['institution'] },
  { key: 'design_experience', label: 'Production & Media Roles', icon: Briefcase, fields: ['year', 'role', 'company', 'location'], primary: 'role', secondary: ['company', 'location'] },
  { key: 'ensembles', label: 'Prior Shows & Networks', icon: Mic, fields: ['year', 'role', 'ensemble', 'location'], primary: 'ensemble', secondary: ['role', 'location'] },
  { key: 'recording_credits', label: 'Guest Appearances & Credits', icon: Headphones, fields: ['year', 'title', 'artist'], primary: 'title', secondary: ['artist'] },
  { key: 'touring', label: 'Live Shows & Tour', icon: Calendar, fields: ['year', 'title', 'venue', 'location'], primary: 'title', secondary: ['venue', 'location'] },
  { key: 'collaborations', label: 'Notable Interviews & Collaborations', icon: Users, fields: ['year', 'title', 'partner'], primary: 'title', secondary: ['partner'] },
  { key: 'awards', label: 'Awards & Honors', icon: Trophy, fields: ['year', 'title', 'organization'], primary: 'title', secondary: ['organization'] },
  { key: 'press', label: 'Press & Features', icon: Newspaper, fields: ['year', 'title', 'publication', 'url'], primary: 'title', secondary: ['publication'], link: 'url' },
  { key: 'commissions', label: 'Sponsored Series & Commissions', icon: BadgeDollarSign, fields: ['year', 'title', 'client'], primary: 'title', secondary: ['client'] },
];

const FIELD_PLACEHOLDERS = {
  year: 'Year',
  degree: 'Broadcasting, Journalism…',
  institution: 'School / Program',
  role: 'Host, Producer, Editor…',
  company: 'Network / Studio',
  location: 'City, State',
  ensemble: 'Show / Network name',
  title: 'Episode / Series title',
  artist: 'Show / Host',
  venue: 'Venue / Festival',
  partner: 'Guest / Partner',
  organization: 'Webby, iHeart…',
  publication: 'Outlet name',
  client: 'Sponsor / Brand',
  url: 'https://…'
};
...
const EMPTY = {
  artist_statement: '',
  education: [], design_experience: [], ensembles: [], recording_credits: [],
  touring: [], collaborations: [], awards: [], press: [], commissions: []
};

export default function PodcastCVTab({ artistId, isOwner, ownerId }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-bold text-foreground">Podcaster CV / Bio</h2>
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Host Statement</p>
        {editing ? (
          <textarea className="w-full px-3 py-2 rounded-lg border border-input bg-transparent text-sm resize-none focus:outline-none focus:ring-1 focus:ring-ring min-h-[120px]"
            placeholder="Write your host statement / show ethos…"
