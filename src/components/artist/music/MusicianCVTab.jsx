...
import {
  GraduationCap, Users, Calendar, Disc, Handshake, Trophy, Newspaper, Radio,
...
const SECTIONS = [
  ...
  { key: 'ensembles', label: 'Ensembles & Bands', icon: Users, fields: ['year', 'role', 'ensemble', 'location'], primary: 'role', secondary: ['ensemble', 'location'] },
  { key: 'touring', label: 'Touring & Performances', icon: Calendar, fields: ['year', 'title', 'venue', 'location'], primary: 'title', secondary: ['venue', 'location'] },
  { key: 'recording_credits', label: 'Recording & Session Credits', icon: Disc, fields: ['year', 'title', 'artist'], primary: 'title', secondary: ['artist'] },
  { key: 'collaborations', label: 'Collaborations & Features', icon: Handshake, fields: ['year', 'title', 'partner'], primary: 'title', secondary: ['partner'] },
  { key: 'awards', label: 'Awards & Nominations', icon: Trophy, fields: ['year', 'title', 'organization'], primary: 'title', secondary: ['organization'] },
...
const FIELD_PLACEHOLDERS = {
  year: 'Year',
  degree: 'BFA Music Performance',
  institution: 'Peabody, Berklee, Juilliard…',
  role: 'Member, Soloist, Bandleader…',
  ensemble: 'Baltimore Symphony, Quartet…',
  location: 'City, State',
  title: 'Show / Album / Track',
  venue: 'Venue / Festival',
  artist: 'Artist / Label',
  partner: 'Featured artist / collaborator',
  organization: 'Grammy, ASCAP, BMA…',
  program: 'Residency / Fellowship',
  publication: 'DownBeat, Rolling Stone…',
  url: 'https://…'
};
...
const EMPTY = {
  artist_statement: '',
  education: [], ensembles: [], touring: [], recording_credits: [],
  collaborations: [], awards: [], press: [], residencies: []
};

export default function MusicianCVTab({ artistId, isOwner, ownerId }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-bold text-foreground">Musician CV / Bio</h2>
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        ...
        {editing ? (
          <textarea className="w-full px-3 py-2 rounded-lg border border-input bg-transparent text-sm resize-none focus:outline-none focus:ring-1 focus:ring-ring min-h-[120px]"
            placeholder="Write your artist statement / bio…"
      ...
      {editing && (
        <div className="bg-card border border-border rounded-xl p-4">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Upload CV / EPK as PDF</p>
