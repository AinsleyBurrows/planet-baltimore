...
export default function PhotoSavedTab({ user }) {
  const { data: savedPostRecords = [] } = useQuery({
    queryKey: ['photo-saved-posts', user?.id],
  ...
  const { data: savedPosts = [] } = useQuery({
    queryKey: ['photo-saved-posts-data', savedPostIds.join(',')],
    queryFn: async () => {
  ...
  const { data: savedStoryRecords = [] } = useQuery({
    queryKey: ['photo-saved-stories', user?.id],
  ...
  const { data: savedStories = [] } = useQuery({
    queryKey: ['photo-saved-stories-data', savedStoryIds.join(',')],
    queryFn: async () => {
