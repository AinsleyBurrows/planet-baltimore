...
export default function ArtistCreatePostModal({ artist, user, onClose }) {
  ...
  const handlePost = async () => {
    ...
    await base44.entities.Post.create({
      ...
      author_avatar: artist.image_url,
      author_type: 'artist',
      page_id: artist.id,
      page_type: 'artist',
      visibility: 'public',
      ...
      >
        ...
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-border flex-shrink-0">
            <h2 className="text-sm font-semibold text-foreground">New Post — {artist.name}</h2>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            >
              <textarea
                ...
                placeholder="Share something on your artist page…"
                rows={4}
              ...
              <div className={`grid gap-2 ${mediaFiles.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
                {mediaFiles.map((m, idx) => (
                  <div key={idx} className="relative rounded-xl overflow-hidden aspect-square bg-secondary">
                    ...
                    <button onClick={() => removeFile(idx)} className="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-black/60 flex items-center justify-center hover:bg-black/80 transition-colors">
            ...
            {isTextOnly && (
              <div className="flex items-center justify-between">
                ...
                {bgColor && <button onClick={() => setBgColor(null)} className="text-xs text-muted-foreground hover:text-foreground underline ml-3 flex-shrink-0">Clear</button>}
              </div>
            )}
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-medium transition-colors ${isPinned ? 'border-accent bg-accent/10 text-accent' : 'border-border text-muted-foreground hover:border-accent hover:text-accent'}`}
            >
