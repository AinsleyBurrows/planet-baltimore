...
import { Plus, Trash2, Pencil, ShoppingBag, Upload, Loader2 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Skeleton } from '@/components/ui/skeleton';

const ITEM_TYPES = ['print', 'original', 'digital_download', 'merch', 'vinyl', 'book', 'other'];
...
export default function ShopTab({ artistId, isOwner }) {
  const queryClient = useQueryClient();
  const { data: items = [], isLoading } = useQuery({
    queryKey: ['shop-items', artistId],
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['shop-items', artistId] });
  const saveNew = async (form) => { await base44.entities.ShopItem.create({ ...form, artist_id: artistId }); setShowForm(false); refresh(); };
