...
export default function GovMessageFollowersModal({ agency, followerCount, onClose }) {
  ...
  const handleSend = async () => {
    ...
    setError('');
    try {
      const res = await base44.functions.invoke('sendGovAgencyMessage', {
        agency_id: agency.id,
        subject,
        message,
        preview_only: false,
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-primary/5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">{followerCount} recipients</span>
          </div>
            ...
            {step === STEPS.COMPOSE && (
              <motion.div key="compose" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                <div>
                  ...
                  <Input className="mt-1" placeholder="Public announcement..." value={subject} onChange={e => setSubject(e.target.value)} />
                </div>
                <div>
                  <Label>Message *</Label>
                  <Textarea className="mt-1" rows={6} placeholder="Write your message to all followers..." value={message} onChange={e => setMessage(e.target.value)} />
            ...
            {step === STEPS.PREVIEW && (
              <motion.div key="preview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 space-y-3">
                  ...
                  <p className="font-semibold text-foreground">{subject || `Message from ${agency.name}`}</p>
                ...
                <div className="flex items-center gap-3 p-3 bg-secondary/50 rounded-xl text-sm text-muted-foreground">
                  ...
                  <span>Will be delivered to <strong className="text-foreground">{followerCount}</strong> followers as in-app notification + inbox message. Email notifications sent to followers with email on file.</span>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <Button onClick={handleConfirm} className="flex-1 gap-2 bg-primary hover:bg-primary/90 text-primary-foreground">
                    <Send className="w-4 h-4" />Send to {followerCount} Followers
                  </Button>
                </div>
              </motion.div>
            )}
              ...
              <motion.div key="confirm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5 text-center py-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    You are about to send an official message from <strong>{agency.name}</strong> to all <strong>{followerCount}</strong> followers. This cannot be undone.
                  </p>
                </div>
                <p className="text-xs text-muted-foreground bg-secondary/50 p-3 rounded-xl">
                  Rate limit: You can send one mass message per hour.
                </p>
                <div className="flex gap-3">
