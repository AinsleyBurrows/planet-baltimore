...
export default function CommunityResourcesTab({ community, isOwner }) {
  ...
  const typeIcon = (t) => t === 'doc' ? <FileText className="w-4 h-4" /> : <LinkIcon className="w-4 h-4" />;
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="bg-card border border-border rounded-xl p-4 space-y-3">
          ...
          <input className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm" placeholder="URL *" value={form.url} onChange={e => setForm(f => ({ ...f, url: e.target.value }))} />
          ...
          <div className="flex gap-2">
            <Button size="sm" onClick={handleAdd} disabled={saving || !form.title.trim() || !form.url.trim()} className="bg-accent hover:bg-accent/90 text-accent-foreground">{saving ? 'Saving…' : 'Add'}</Button>
          ...
          {resources.map((r, i) => (
            <div key={i} className="flex items-start gap-3 p-4 bg-card border border-border rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                ...
                <a href={r.url} target="_blank" rel="noopener noreferrer" className="text-xs text-accent hover:underline flex items-center gap-1 mt-1">
