        ...
        <div className="p-5">
          ...
          <div className="flex items-center gap-3 mt-4">
            <Avatar className="w-8 h-8">
              ...
              <AvatarFallback className="bg-accent/10 text-accent text-xs">{story.author_name?.charAt(0)}</AvatarFallback>
            </Avatar>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-2">
          <Avatar className="w-6 h-6">
            <AvatarImage src={story.author_avatar} />
            <AvatarFallback className="bg-accent/10 text-accent text-[10px]">{story.author_name?.charAt(0)}</AvatarFallback>
          </Avatar>
          <span className="text-xs font-medium text-foreground">{story.author_name}</span>
          {story.category && <Badge variant="secondary" className="text-[10px] capitalize">{story.category}</Badge>}
        </div>
        <h3 className="font-serif font-semibold text-foreground group-hover:text-accent transition-colors line-clamp-2">{story.title}</h3>
        {story.subtitle && <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{story.subtitle}</p>}
        <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
          <div className="flex items-center gap-1"><Clock className="w-3 h-3" />{story.reading_time || 5} min</div>
          <div className="flex items-center gap-1"><Heart className="w-3 h-3" />{story.likes_count || 0}</div>
          <div className="flex items-center gap-1"><MessageCircle className="w-3 h-3" />{story.comments_count || 0}</div>
        </div>
      </div>
      {story.cover_image && (
