    ...
    <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4 sm:p-8" onClick={onClose}>
      <div className="relative flex flex-col sm:flex-row max-w-5xl w-full max-h-full bg-card rounded-2xl overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="relative flex-1 flex items-center justify-center min-h-0 bg-black min-h-[40vh] sm:min-h-0">
          ...
          {posts.length > 1 && (
            <>
              <button onClick={() => go(-1)} className="absolute left-3 top-1/2 -translate-y-1/2 p-3 rounded-full bg-black/40 text-white hover:bg-black/60 backdrop-blur-sm"><ChevronLeft className="w-6 h-6" /></button>
              <button onClick={() => go(1)} className="absolute right-3 top-1/2 -translate-y-1/2 p-3 rounded-full bg-black/40 text-white hover:bg-black/60 backdrop-blur-sm"><ChevronRight className="w-6 h-6" /></button>
            </>
          )}
        </div>
