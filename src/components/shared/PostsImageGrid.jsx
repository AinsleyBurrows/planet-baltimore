...
function Viewer({ posts, index, onClose, onIndex }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/90 flex flex-col sm:flex-row" onClick={onClose}>
      {/* Image / media side */}
      <div className="relative flex-1 flex items-center justify-center p-4 sm:p-8 min-h-0">
        {m.kind === 'image' && (
          <img src={m.url} alt={post.content || ''} className="max-w-full max-h-full object-contain" onClick={e => e.stopPropagation()} />
        )}
        {m.kind === 'video' && (
          <video src={post.media_urls[0]} poster={m.thumb || undefined} controls className="max-w-full max-h-full" onClick={e => e.stopPropagation()} />
        )}
        {m.kind === 'audio' && (
        ...
        {m.kind === 'color' && (
          <div className="w-full max-w-md aspect-video flex items-center justify-center p-8 rounded-lg" style={{ backgroundColor: m.color }} onClick={e => e.stopPropagation()}>
            <p className="font-serif text-xl leading-snug text-center" style={{ color: '#fff' }}>{m.text}</p>
          </div>
        )}
        {m.kind === 'text' && (
          <div className="w-full max-w-md p-8 bg-card rounded-lg" onClick={e => e.stopPropagation()}>
        ...
        {posts.length > 1 && (
          <>
            <button onClick={(e) => { e.stopPropagation(); go(-1); }} className="absolute left-3 sm:left-5 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/10 text-white hover:bg-white/20 backdrop-blur-sm"><ChevronLeft className="w-6 h-6" /></button>
            <button onClick={(e) => { e.stopPropagation(); go(1); }} className="absolute right-3 sm:right-5 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/10 text-white hover:bg-white/20 backdrop-blur-sm"><ChevronRight className="w-6 h-6" /></button>
          </>
        )}
      </div>

      {/* Info side panel */}
      <aside className="sm:w-80 lg:w-96 bg-card overflow-y-auto border-t sm:border-l border-white/10 max-h-[40vh] sm:max-h-full" onClick={e => e.stopPropagation()}>
