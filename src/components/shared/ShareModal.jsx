...
export default function ShareModal({ isOpen, onClose, url, title, description }) {
  ...
  const handleInstagramStories = async () => {
    ...
    // Try to open Instagram Stories camera via deep link (works on mobile)
    window.location.href = 'instagram://story-camera';
    // Fallback to Instagram app after short delay
    setTimeout(() => {
  ...
  const handleSocialShare = (platform) => {
    if (platform.isInstagramStories) { handleInstagramStories(); return; }
        ...
        >
          ...
          >
            ...
            <div className="p-5 space-y-5">
              ...
              {/* Instagram Stories hint */}
              {igCopied && (
                <div className="flex items-start gap-2 bg-gradient-to-r from-[#f09433]/10 to-[#bc1888]/10 border border-[#e6683c]/30 rounded-xl px-3 py-2.5">
                  <span className="text-lg leading-none">📋</span>
                  <div>
                    <p className="text-xs font-semibold text-foreground">Link copied!</p>
                    <p className="text-[11px] text-muted-foreground leading-snug">Opening Instagram… paste the link as a sticker in your Story.</p>
                  </div>
                </div>
              )}
