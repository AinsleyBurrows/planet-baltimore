...
export default function BusinessMessageModal({ business, onClose }) {
  ...
  const handleSend = async () => {
    ...
    await Promise.all(emails.map(email =>
      base44.integrations.Core.SendEmail({
        ...
        body: `${message}\n\n—\nThis message was sent to all followers of ${business.name} on Planet Baltimore.`,
      }).catch(() => {})
    ));
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-border flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1">
              <Users className="w-3 h-3" /> Sends to all followers of {business.name}
            </p>
          </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={`A message from ${business.name}`}
                  value={subject}
              ...
              <p className="text-xs text-muted-foreground">Followers will receive this as an email. Your business name will be included.</p>
            </div>
