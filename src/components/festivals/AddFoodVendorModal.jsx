...
export default function AddFoodVendorModal({ open, onOpenChange }) {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      return base44.entities.FoodVendor.create({
        name: name.trim(),
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['food-vendors'] });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Add a Food Vendor</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Charm City Tacos" />
          </div>
          <div>
          ...
          <div>
            ...
            <Textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} placeholder="What do they serve?" />
          </div>
          <div>
            <Label className="mb-1.5 block">Menu / Website (optional)</Label>
